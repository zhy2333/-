...
class ImageClassifierApp(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.nInput.setPlaceholderText("请输入样本分组数量 (例如 10)")
        # 标记样本图像按钮
        self.showSampleBtn = QtWidgets.QPushButton("标记样本", self)
        ...
        grid.addWidget(QLabel("样本分组数量 (n)："), 3, 1, 1, 2)
        grid.addWidget(self.nInput, 3, 3, 1, 10)
    ...
    def getFinal(self,type00, n, max):
        ...
        for i in range(n):
            list3 = [random.randint(0, len(bin_list[i]) - 1) for _ in
                     range(int(0.0005 * len(bin_list[i]) * len(bin_list[i]) / num))]
